...
#define LCD_DISPLAY_ON_OFF 		0b00001000
	...
	#define	DISPLAY_CURSOR_BLNK	0b00000001
...
#define ROW2_OFFSET_ADDR	0x40
#define ROW3_OFFSET_ADDR	0x14
/*ase 0: r=0x00; break;
	  case 1: r=0x40; break;
	  case 2: r=0x14; break;
	  case 3: r=0x54;  break;*/
...
LCDCharacterDisplay::LCDCharacterDisplay(SPIDevice *device, int width, int height) {
	...
	this->displayState = LCD_DISPLAY_ON_OFF | DISPLAY_ON ;
...
int LCDCharacterDisplay::setCursorPosition(int row, int column){
    ...
    rowoffset=(row%2)*ROW2_OFFSET_ADDR + (row/2)*ROW3_OFFSET_ADDR;
    int value = (rowoffset + column) | LCD_DDRAM_ADDR;
	this->command(value);
	//printf("[%02x]", value);
    return 0;
}
...
void LCDCharacterDisplay::setDisplayOff(bool displayOff){
	if (displayOff){
		this->displayState = displayState & (~DISPLAY_ON); //bit inversion of DISPLAY_ENTIRE
		this->writeDisplayState();
	}
	else{
		this->displayState = displayState | DISPLAY_ON;
		this->writeDisplayState();
	}
}
...
void LCDCharacterDisplay::setCursorBlink(bool isBlink){
	if (!isBlink){
		this->displayState = displayState & (~DISPLAY_CURSOR_BLNK); //bit inversion of DISPLAY_ENTIRE
		this->writeDisplayState();
	}
	else{
		this->displayState = displayState | DISPLAY_CURSOR_BLNK;
		this->writeDisplayState();
	}
}
